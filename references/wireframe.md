...
Load `wireframe-visual-language.md`, `dopamine-component-catalog.md`, and `component-composition-recipes.md` before constructing the artifact.

Use the live catalogue to inspect only the components required by the agreed screen, surface, and states. Confirm the relevant ready component, variant, size, and state before recreating it in the wireframe. Do not browse the complete library. If the catalogue is unavailable after one attempt, continue with `dopamine-2-wireframe-reference.md` and supplied product sources.
...
When no product screen is supplied, use the live component catalogue plus the default Dopamine construction system in `wireframe-visual-language.md` and `dopamine-2-wireframe-reference.md`.
